...
from src.pyiak_instr.encoders.bin import BytesIntDecoder
...
class TestDataLengthMessageFieldStruct(unittest.TestCase):
    ...
    def test_calculate(self) -> None:
        with self.subTest(test="U8 BYTES"):
            self.assertEqual(
                ...
                TIDynamicLength().calculate(
                    b"a" * 20, BytesIntDecoder(fmt=Code.U8).fmt_bytesize,
                ),
            )
        ...
        with self.subTest(test="U16 BYTES"):
            self.assertEqual(
                ...
                TIDynamicLength(
                    units=Code.BYTES
                ).calculate(b"a" * 20, BytesIntDecoder(fmt=Code.U16).fmt_bytesize)
            )
        ...
        with self.subTest(test="U32 WORDS"):
            self.assertEqual(
                ...
                TIDynamicLength(
                    units=Code.WORDS
                ).calculate(b"a" * 16, BytesIntDecoder(fmt=Code.U32).fmt_bytesize)
            )
    ...
    def test_calculate_exc(self) -> None:
        with self.assertRaises(ContentError) as exc:
            TIDynamicLength(
                units=Code.WORDS
            ).calculate(b"a" * 5, BytesIntDecoder(fmt=Code.U32).fmt_bytesize)
        self.assertEqual(
